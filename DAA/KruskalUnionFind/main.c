...
/*
    set_t * k_vertices = createSet();

    int k_edges[MAX_ARR_SIZE];
    int ke_len = 0;

    setDisplay(k_vertices);
    for (int i = 0; i < num_edges; i++) {
        printf("%c -> %c (%d)\n", *edges[i].from, *edges[i].to, edges[i].weight);
        if(!setIncludes(k_vertices, edges[i].to) && !setIncludes(k_vertices, edges[i].from)) {
            k_edges[ke_len++] = i;
            setInclude(k_vertices, edges[i].to);
            setInclude(k_vertices, edges[i].from);
        } else if (!setIncludes(k_vertices, edges[i].to)) {
            k_edges[ke_len++] = i;
            setInclude(k_vertices, edges[i].to);
        }
        setDisplay(k_vertices);
    ...
    for(int i = 0; i < ke_len; i++) {
        printf("%d\t", edges[k_edges[i]].weight);
    }
*/
...
int k_edges[MAX_ARR_SIZE];
    ...
    for(int i = 0; i < num_edges && k_edges_len < _vertices->len - 1; i++) {
        if(setIncludes(k_vertices, edges[i].to)) continue;
        k_edges[k_edges_len++] = i;
        setInclude(k_vertices, edges[i].from);
        setInclude(k_vertices, edges[i].to);
    }
