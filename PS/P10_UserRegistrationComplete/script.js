 ...
 * @param {MouseEvent} e 
 */
function buttonOrAnchorHandler(e) {
    e.preventDefault();
    const pageTitle = e.target.getAttribute('data-render');
    render(pageTitle);
}
...
function buttonActionHandler(e) {
    ...
    if (action == 'login') {
        ...
        if (currentUserName == USERNAME && currentPassword == PASSWORD) {
            ...
            render('invalid-prompt')
        }
    }
    else if (action == 'signup') {
        alert("This is a demo, no signup is required, logging in...");
        render('login-success')
    }
        ...
        alert(`This is a demo, no password reset is required, \nUN:${USERNAME} \nPW:${PASSWORD}`);
        render('login-page');
    }
}
...
function render(id) {
    ...
    main.innerHTML = "";
    main.appendChild(document.querySelector(`#${id}`).content.cloneNode(true));
    main.setAttribute('data-page', id);

    document.querySelector('.sub-title').innerHTML = id;

    // Hooking methods for transitions b/w pages
    document.querySelectorAll('button[data-render]').forEach((button) => {
        button.addEventListener('click', buttonOrAnchorHandler);
    });
    document.querySelectorAll('a[data-render]').forEach((anchor) => {
        anchor.addEventListener('click', buttonOrAnchorHandler);
    });
    document.querySelectorAll('button[data-action]').forEach((button) => {
        button.addEventListener('click', buttonActionHandler);
    });
